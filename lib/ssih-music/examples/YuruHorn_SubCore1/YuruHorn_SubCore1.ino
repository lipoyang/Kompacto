...
const int VOLUME_THRESH = 800;
const int FREQ_MIN = 80;
const int FREQ_MAX = 1100;
...
uint16_t getInputLevel(const int16_t *input, int length) {
    uint32_t sum = 0;
    int count = 0;
...
float fftProcessing() {
    ...
    if (peakFs < FREQ_MIN || FREQ_MAX < peakFs) {
        peakFs = 0;
    }
    // printf("peakFs:%8.3f\n", peakFs);
    return peakFs;
